...
class HForm(Form):
    ...
    h_disk_type = SelectField(u'磁盘类型', choices=[(1, 'SAS'), (2, 'SSD'), (3, 'SATA')], coerce=int)
    h_disk_num = IntegerField(u'磁盘数量', [validators.required('磁盘数量需为整数')])
    h_disk_size = IntegerField(u'单盘容量', [validators.required('单盘容量需为整数')])
    h_raid = StringField(u'Raid')
    h_eth = SelectField(u'网卡设备', choices=[(1, 'eth0'), (2, 'em0')], coerce=int)
    h_ip = StringField(u'网卡IP', [validators.required('网卡ip必须填写！'), validators.length(max=15)])
    h_vip = StringField(u'虚拟IP', [validators.length(max=15)])
    h_remote_ip = StringField(u'远控卡IP', [validators.length(max=15)])
    h_idc = SelectField(u'IDC', choices=[(1, '天津IDC'), (2, 'IDC10'), (3, 'IDC20')], coerce=int)
...
class InstanceForm(Form):
    ...
    i_hostname = StringField(u'机器名', [validators.required(u'机器名必须填写！'), validators.length(max=32)])
