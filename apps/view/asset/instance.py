...
def add_mysql_instance():
    ...
    form.i_base.i_role.choices = [(1, 'Master'), (2, 'Slave')]
    ...
    if request.method == 'POST' and form.validate():
        ...
        if not r_hid:
            ...
            hid = r_hid[0]
            check_ins = aservice.find_mysql_instance_by_name_and_port(gid, hid, port)
            if not check_ins:
                mi = MysqlInstance(gid, hid, port, role, status, ver, remark)
                aservice.add(mi)
...
def add_mc_instance():
    ...
    form.i_base.i_role.choices = [(1, 'Master'), (2, 'Slave'), (3, 'StandAlone')]
    form.i_base.i_name.choices = group_names
    if request.method == 'POST' and form.validate():
        gid = form.i_base.i_name.data
        ver = form.i_base.i_version.data
        ...
        if not r_hid:
            ...
            hid = r_hid[0]
            check_ins = aservice.find_mc_instance_by_name_and_port(gid, hid, port)
            if not check_ins:
                mi = McInstance(gid, hid, port, mem, thread, con, factor, para, role, status, muser, ver, remark)
                aservice.add(mi)
                flash(u"Add Success!", 'info')
                return redirect(url_for('.instances'))
            else:
...
def add_redis_instance():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if not r_hid:
            ...
            check_ins = aservice.find_redis_instance_by_name_and_port(gid, hid, port)
            if not check_ins:
                mi = RedisInstance(gid, hid, port, mem, pre, role, status, ver, remark)
                aservice.add(mi)
                flash(u"Add Success!", 'info')
                return redirect(url_for('.instances'))
            else:
